...
class NLPCA( PrepObj ):
  ...
  def __init__(self,n_inits=1,n_nlpcas=30, n_neurons_mapping=50, hidden_activation='tanh',output_activation='linear',n_epochs=10000,patience=30,batch_size=200,layer=1, d = {}, **kw):
    d.update( kw ); del kw
    from RingerCore import retrieve_kw
    #self._caltype = retrieve_kw(d,'caltype','allcalo')
    self._n_nlpcas = retrieve_kw(d,'n_nlpcas',40)
    self._n_neurons_mapping = retrieve_kw(d,'n_neurons_mapping',60)
    #self._aetype = retrieve_kw(d,'aetype','vanilla') #VANILLA, SPARSE, DENOISING, CONTRACTIVE
    #self._dataEncoded = retrieve_kw(d,'dataEncoded','all') #all, signal, background
    PrepObj.__init__( self, d )
    checkForUnusedVars(d, self._warning )
