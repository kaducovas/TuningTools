...
def reconstruct_performance(norm1Par=None,reconstruct=None,model_name="",time=None,sort=None,etBinIdx=None,etaBinIdx=None,phase=None,lstm_target=None,measure='Normalized_MI',Normed=False):
  ...
  db = dataset.connect('sqlite:////scratch/22061a/caducovas/run/ringer_new.db')
  #print point.sp_value
  table = db['reconstruction_metrics3']
  metrics = OrderedDict()
...
def plot_input_reconstruction_delta(norm1Par=None,reconstruct=None,model_name=None,layer=None,time=None, etBinIdx=None,etaBinIdx=None,log_scale=False, dirout=None):
  ...
  ax2 = ax.twinx()
  ax2.plot(np.arange(beforenorm_val_Data.shape[1]), (np.mean(unnorm_reconstruct_val_Data, axis=0) - np.mean(beforenorm_val_Data, axis=0))/np.std(beforenorm_val_Data, axis=0), marker='<', color='cornflowerblue')
...
##TEM DOIS IDENTS QUE TEM QUE TIRAR. O DO FOR LAYER E O DO IF IS INSTANCES
    ...
    ax2.legend(loc='best', fontsize='xx-large')
    ax22 = ax2.twinx()
    ax22.plot(np.arange(beforenorm[1].shape[1]), (np.mean(unnorm_reconstruct[1], axis=0) - np.mean(beforenorm[1], axis=0))/np.std(beforenorm[1], axis=0), marker='>', color='cornflowerblue')
