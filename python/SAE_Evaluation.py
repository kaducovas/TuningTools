...
def getLSTMReconstruct(norm1Par,sort,model_name=None):
  ...
  if isinstance(afternorm, (tuple, list,)):
    ...
    for i, cdata in enumerate(afternorm):
      print i,cdata.shape
      reconstruction = wrapper.generate_np_reconstruction(model_filename=Path('/home/users/caducovas/deep_output/ringer_1_1_24_epochs_1_layer_30_units/logs/model'),
                                                          global_step=None,
                                                          data_set=cdata,
                                                          batch_size=10000)
